...
@ROBOTS.register()
class FourWheelRearSteerRobot_V1(FourWheeledRobot):
    """Four-wheeled robot with rear-axle (forklift-style) steering.

    Uses the Isaac Sim Forklift C asset.
    """
    ...
    chase_camera_z_offset: float = 1.5
    chase_camera_tilt_angle: float = 60.0

    occupancy_map_radius: float = 1.5
    occupancy_map_z_min: float = 0.1
    occupancy_map_z_max: float = 2.5
    occupancy_map_cell_size: float = 0.05
    occupancy_map_collision_radius: float = 1.0

    main_camera_base_path = "body/front_camera"
    main_camera_rotation = (0.0, 0.0, 0.0)
    main_camera_translation = (0.0, 0.0, 0.0)
    main_camera_type = HawkCamera

    keyboard_linear_velocity_gain: float = 1.0
    keyboard_angular_velocity_gain: float = 0.6

    gamepad_linear_velocity_gain: float = 1.0
    gamepad_angular_velocity_gain: float = 0.6

    random_action_linear_velocity_range: Tuple[float, float] = (-0.3, 1.0)
    random_action_angular_velocity_range: Tuple[float, float] = (-0.5, 0.5)
    random_action_linear_acceleration_std: float = 3.0
    random_action_angular_acceleration_std: float = 2.0
    random_action_grid_pose_sampler_grid_size: float = 5.0

    path_following_speed: float = 0.8
    path_following_angular_gain: float = 1.0
    path_following_stop_distance_threshold: float = 0.8
    path_following_forward_angle_threshold = math.pi / 4
    path_following_target_point_offset_meters: float = 1.5
